...
public class Suitcase {
	...
	public Thing heaviestThing() {
		Thing buffer = new Thing(this.things.get(0).getName(), this.things.get(0).getWeight());
		for (Thing thing : things) {
			if (thing.getWeight() > buffer.getWeight()) {
				heaviest = new Thing(thing.getName(), thing.getWeight());
			}
			else {
				continue;
			}
